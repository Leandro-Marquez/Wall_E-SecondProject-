...
class Assistant
{
    ...
    public static object EvaluateSpawn(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 2)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of Spawn()" + $" Error_Line : {Context.indexOfEvaluation + 1}"));
            return null;
        }
        else //en caso contrario 
        {
            ...
            if(x is int xValue && y is int yValue) //si ambos son enteros no se tiene error, por lo q se ejecutara correctamente 
            {
                ...
                else Error.errors.Add((ErrorType.Run_Time_Error, "Spawn() parameters must be positive and less than Canvas'Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            }
            else //si al menos uno no es entero
            {
                // Mensaje de error detallado
                string errorMsg = "Invalid types in 'Spawn': ";
                if (x is not int) errorMsg += $"X must be int, but got '{x?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (y is not int) errorMsg += $"Y must be int, but got '{y?.GetType().Name}'." + $" Error_Line : {Context.indexOfEvaluation+ 1}";
    ...
    public static object EvaluateColor(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 1)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of Color()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;
        }
        else //en caso contrario
        {
            ...
            {
                Error.errors.Add((ErrorType.Run_Time_Error, $"Color's Method must receive Int's Type and recibe {a.GetType()}" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
                return null;
            }
            else //en caso contrario verificar q color es el q se pide para cambiar el color 
            {
                switch (a) //switch case con cada color
                {
                    ...
                    default: //en otro caso se tiene un error en tiempo de ejecucion, color invalido
                        Error.errors.Add((ErrorType.Run_Time_Error, $"{a} is not a valid Color Type" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
                        break;
                }
            }
        }
    ...
    public static object EvaluateGetCanvasZize(List<ASTNode> Params)
    {
        ...
        if(Params.Count > 0)
        {
            Error.errors.Add((ErrorType.Run_Time_Error , "GetCanvasZize() does not contains parameters" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
    ...
    public static object EvaluateGetActualX(List<ASTNode> Params)
    {
        ...
        if(Params.Count > 0)
        {
            Error.errors.Add((ErrorType.Run_Time_Error, "GetActualX() does not contains parameters" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
    ...
    public static object EvaluateGetActualY(List<ASTNode> Params)
    {
        ...
        if(Params.Count > 0)
        {
            Error.errors.Add((ErrorType.Run_Time_Error, "GetActualY() does not contains parameters" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
    ...
    public static object EvaluateDrawLine(List<ASTNode> Params)
    {
        ...
        if (Params.Count != 3)
        {
            Error.errors.Add((ErrorType.Run_Time_Error, "There is no argument given that corresponds to the required parameter of DrawLine()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        ...
        for (var i = 0; i < Params.Count; i++)
        {
            ...
            {
                Error.errors.Add((ErrorType.Run_Time_Error, $"DrawLine's Method must receive Int's Type and recibe {a.GetType()}" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
        ...
        {
            Error.errors.Add((ErrorType.Run_Time_Error, "Distance and brush size must be positive integers" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
    ...
    public static object EvaluateSize(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 1)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of Size()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            if(a is int) //si es entero
            {
                if((int)a == 0)
                {
                    Error.errors.Add((ErrorType.Run_Time_Error, "BrushSize's must be a positive integer" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
                    return null;//retornar null ya q hubo error
                }
                int auxiliar = 0;
            ...
            else Error.errors.Add((ErrorType.Run_Time_Error,"Size's Method must recibe Int's Type" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
        }
        return null; //retornar null ya q es un metodo void
    ...
    public static object EvaluateIsBrushSize(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 1)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of IsBrushSize()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            else Error.errors.Add((ErrorType.Run_Time_Error,"IsBrushSize's Method must recibe Int's Type" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
        }
    ...
    public static object EvaluateIsBrushColor(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 1)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of IsBrushColor()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            else Error.errors.Add((ErrorType.Run_Time_Error,"IsBrushSize's Method must recibe String's Type" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
        }
    ...
    public static object EvaluateFill(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 0)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"Fill's Method does not contains params" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
    ...
    public static object EvaluateGetColorCount(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 5)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of GetColorCount()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            if(x1 is int x1Value && y1 is int y1Value && x2 is int x2Value && y2 is int y2Value && color is string colorValue)
            {
                ...
                if(x1Value >= 0 && x1Value < Context.canvasSize && y1Value >= 0 && y1Value < Context.canvasSize && x2Value >= 0 && x2Value < Context.canvasSize && y2Value >= 0 && y2Value < Context.canvasSize)
                {
                    ...
                    switch (color) //hacer switch con cada color para verificar si es un color 
                    {
                        ...
                        default: //si se llega aqui es q no se tiene un color valido
                            Error.errors.Add((ErrorType.Run_Time_Error,$"{color} is not a valid Color Type" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
                            return null;//retornar null ya q hubo error
                    }
                ...
                else Error.errors.Add((ErrorType.Run_Time_Error, "GetColorCount() parameters must be positive and less than Canvas'Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            }
            else //en cualquier otro caso, se tiene un error de tipos
            {
                string errorMsg = "Invalid types in GetColorCount: ";
                if (x1 is not int) 
                    errorMsg += $"X1 must be int, but got '{x1?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (y1 is not int) 
                    errorMsg += $"Y1 must be int, but got '{y1?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (x2 is not int) 
                    errorMsg += $"X2 must be int, but got '{x2?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (y2 is not int) 
                    errorMsg += $"Y2 must be int, but got '{y2?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (color is not string) 
                    errorMsg += $"Color must be String, but got '{color?.GetType().Name}'." + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                Error.errors.Add((ErrorType.Run_Time_Error, errorMsg));
            } 
        }
    ...
    public static object EvaluateIsCanvasColor(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 3)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of IsCanvasColor()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            if(color is string colorValue && x is int xValue && y is int yValue)
            {
                ...
                //en caso contrario se tiene un error en tiempo de ejecucion
                else Error.errors.Add((ErrorType.Run_Time_Error,"IsCanvasColor() parameters [x,y] must be positive and less than Canvas'Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            }
            else //en cualquier otro caso, se tiene un error de tipos
            {
                string errorMsg = "Invalid types in IsCanvasColor: ";
                if (x is not int) 
                    errorMsg += $"X must be int, but got '{x?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (y is not int) 
                    errorMsg += $"Y must be int, but got '{y?.GetType().Name}'." + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (color is not string) 
                    errorMsg += $"Color must be string, but got '{color?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                Error.errors.Add((ErrorType.Run_Time_Error, errorMsg));
            }
    ...
    public static object EvaluateDrawCircle(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 3)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of DrawCircle()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            { 
                ...
                if(((int)x == 1 || (int)x == -1 || (int)x == 0 ) && ((int)y == 1 || (int)y == -1 ||(int)y == 0))
                {
                    ...
                    {
                        Error.errors.Add((ErrorType.Run_Time_Error , $"Wall_E position would be out of range" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            ...
            else //en caso contrario se tiene un error de tipos
            {
                ...
                if (x is not int) 
                    errorMsg += $"dirX must be int, but got '{x?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (y is not int) 
                    errorMsg += $"dirY must be int, but got '{y?.GetType().Name}'." + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (radius is not int) 
                    errorMsg += $"radius must be int, but got '{radius?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (radius is int && (int)radius < 0)
                    errorMsg += "radius must be positive and less than Canvas's Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                Error.errors.Add((ErrorType.Run_Time_Error, errorMsg));
            }
    ...
    public static object EvaluateDrawRectangle(List<ASTNode> Params)
    {
        ...
        if(Params.Count != 5)
        {
            Error.errors.Add((ErrorType.Run_Time_Error,"There is no argument given that corresponds to the required parameter of DrawRectangule()" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
            return null;//retornar null ya q hubo error
        }
        else //en caso contrario
        {
            ...
            else //en otro caso dar un mensaje de error detallado
            {
                string errorMsg = "Invalid types in DrawCircle: ";
                if (dirX is not int) 
                    errorMsg += $"dirX must be int, but got '{dirX?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (dirX is int && (int)dirX != 1  && (int)dirX != -1  && (int)dirX != 0 ) 
                    errorMsg += $"any valid direction value must be 1,-1,0" + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (dirY is not int) 
                    errorMsg += $"dirY must be int, but got '{dirY?.GetType().Name}'." + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (dirY is int && (int)dirY != 1  && (int)dirY != -1  && (int)dirY != 0 ) 
                    errorMsg += $"any valid direction value must be 1,-1,0" + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (distance is not int) 
                    errorMsg += $"Distance must be int, but got '{distance?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if(distance is int && (int)distance < 0)
                    errorMsg += "Distance must be positive and less than Canvas's Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (width is not int) 
                    errorMsg += $"Width must be int, but got '{width?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if(width is int && (int)width < 0)
                    errorMsg += "Widht must be positive and less than Canvas's Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if (heigth is not int) 
                    errorMsg += $"Height must be int, but got '{heigth?.GetType().Name}'. " + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                if(heigth is int && (int)heigth < 0)
                    errorMsg += "Heigth must be positive and less than Canvas's Size" + $" Error_Line : {Context.indexOfEvaluation+ 1}";
                Error.errors.Add((ErrorType.Run_Time_Error, errorMsg));
            }
        }
    ...
    private static object Divide(object a, object b)//metodo auxiliar para dividir
    {
        if (a is int aInt && b is int bInt) //si ambos son enteros
        {
            ...
            {
                Error.errors.Add((ErrorType.Run_Time_Error, "Division by zero" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
                return null;
            }
    ...
    private static bool AreEqual(object left, object right , Token Operator) //metodo auxiliar para verificar si se tiene los mismos tipos en los operandos
    {
        ...
        if (left.GetType() != right.GetType()) Error.errors.Add((ErrorType.Run_Time_Error, $"Operator '{Operator.Value}' cannot be applied to operands of type {GetTypeName(left)} and {GetTypeName(right)}" + $" Error_Line : {Context.indexOfEvaluation+ 1}"));
